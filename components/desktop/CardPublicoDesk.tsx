...
import BordaGradiente from "@/components/BordaGradiente";

interface CardPublicoDeskProps {
  titulo: string;
  descricao: string;
  icone: React.ReactNode;
}

export default function CardPublicoDesk({
  titulo,
  descricao,
  icone,
}: CardPublicoDeskProps) {
  return (
    <div className="overflow-hidden mx-auto pl-3.5">
      <div className="border-purple2 bg-purple4 relative rounded-md border-2 py-1 pr-3 pl-5">
        ...
        <div className="relative flex gap-3">
          ...
          <div className="flex flex-col pl-18">
            <h3 className="text-yellow3 pb-1 text-base font-bold xl:text-3xl">{titulo}</h3>
            <p className="text-base text-white/85 xl:text-xl">{descricao}</p>
          </div>
        </div>
        <BorderBeam
          duration={6}
